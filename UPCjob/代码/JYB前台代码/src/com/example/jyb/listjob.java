...
public class listjob extends Activity implements OnItemClickListener{
	int width;
	int height;
...
//Toast.makeText(getApplicationContext(), "ss"+in.getStringExtra("result"), 1).show();
        	...
        	SimpleAdapter adapter = new SimpleAdapter(listjob.this, data, R.layout.item1,
	  ...
	  private  List<job> parseJSON(String  json) throws Exception{
	  		...
	  		for(int i = 0; i < jsonArray.length() ; i++){
	  			...
	  			list.add(new job(title, name, content,"fff",kind));
